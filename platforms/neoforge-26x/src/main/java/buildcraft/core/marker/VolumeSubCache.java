 ...
 * own class javadoc). So this constructor loads from {@link VolumeSavedData#TYPE} only when {@code level} is
...
public class VolumeSubCache extends MarkerSubCache<VolumeConnection> {
    public VolumeSubCache(Level level) {
        ...
        if (level instanceof ServerLevel serverLevel) {
            VolumeSavedData data = serverLevel.getDataStorage().computeIfAbsent(VolumeSavedData.TYPE);
            data.loadInto(this);
        }
    }
