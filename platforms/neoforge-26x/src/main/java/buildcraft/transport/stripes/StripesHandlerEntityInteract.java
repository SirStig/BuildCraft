 ...
 * shearing through the item's own interaction, ...). {@code EntityPlayer#interactOn(Entity, EnumHand)} gained a
 * third {@code Vec3} "where on the entity" parameter on this target; the entity's own base position is passed,
 * matching what a stripes pipe (which has no real click point of its own) can offer. */
public enum StripesHandlerEntityInteract implements IStripesHandlerItem {
    ...
    public boolean handle(
        ...
        for (LivingEntity entity : entities) {
            if (player.interactOn(entity, InteractionHand.MAIN_HAND, entity.position()) instanceof InteractionResult.Success) {
                return true;
            }
        }
        return false;
