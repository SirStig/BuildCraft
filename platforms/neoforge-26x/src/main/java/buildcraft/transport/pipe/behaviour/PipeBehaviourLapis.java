...
/**
 * The lapis pipe's real 1.12.2 mechanic, confirmed by reading {@code PipeBehaviourLapis} directly: it does not
 * move items any differently at all. It simply paints every item that reaches its centre with a
 * {@link DyeColor} (one of 16, wrench-cycled -- right-click advances one step, sneak-right-click steps
 * backwards), for a diamond-style colour-sorting pipe further down the network to filter on. This port has no
 * colour-filtering pipe (diamond) yet and no redstone-gate/statement system at all (see
 * {@code buildcraft.api.transport.pipe.PipeEventStatement}'s non-use throughout this whole module), so
 * {@code ActionPipeColour} (a red-pipe-wire action that could also set the colour) is not ported -- wrench
 * cycling is this port's only way to set it, exactly like {@link PipeBehaviourDaizuli}.
 */
