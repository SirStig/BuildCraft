...
/** Port of 1.12.2's {@code buildcraft.silicon.gate.GateVariant}. {@code NBTTagCompound} is {@link CompoundTag};
 * {@code PacketBuffer} is plain {@link FriendlyByteBuf} here rather than {@code RegistryFriendlyByteBuf} -- a
 * variant is three plain enum ordinals, nothing registry-backed. */
public class GateVariant {
    ...
    public GateVariant(CompoundTag nbt) {
        this.logic = EnumGateLogic.getByOrdinal(nbt.getByteOr("logic", (byte) 0));
        this.material = EnumGateMaterial.getByOrdinal(nbt.getByteOr("material", (byte) 0));
        this.modifier = EnumGateModifier.getByOrdinal(nbt.getByteOr("modifier", (byte) 0));
        this.numSlots = material.numSlots / modifier.slotDivisor;
        this.numTriggerArgs = modifier.triggerParams;
        this.numActionArgs = modifier.actionParams;
        this.hash = Objects.hash(logic, material, modifier);
