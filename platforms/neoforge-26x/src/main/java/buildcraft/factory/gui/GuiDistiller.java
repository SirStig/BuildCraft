 ...
 *
 * <p><b>Fluid bars</b> use {@link GuiAutoCraftFluids}' verified technique (the fluid's still sprite and nullable
 * tint from {@code FluidStateModelSet}, clipped with {@code enableScissor}), with one change: 1.12.2's
...
public class GuiDistiller extends AbstractContainerScreen<ContainerDistiller> {
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath("buildcraft", "textures/gui/distiller.png");
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (tile.isActive()) {
            ...
            int inputColour = fluidColour(tile.tankIn.getFluidType());
            int liquidColour = fluidColour(tile.tankLiquidOut.getFluidType());
            int gasColour = fluidColour(tile.tankGasOut.getFluidType());
            ...
            if (!gasBlocking && !currentGas.isEmpty() && recipe != null
                && !FluidStack.isSameFluidSameComponents(recipe.outGas(), currentGas)) {
                gasBlocking = true;
            }
            if (gasBlocking) {
            ...
            if (!liquidBlocking && !currentLiquid.isEmpty() && recipe != null
                && !FluidStack.isSameFluidSameComponents(recipe.outLiquid(), currentLiquid)) {
                liquidBlocking = true;
            }
            if (liquidBlocking) {
    ...
    private void tankTooltip(GuiGraphicsExtractor graphics, Tank tank, int[] rect, String titleKey, int mouseX, int mouseY) {
        ...
        Component name = fluid.isEmpty() ? Component.translatable("buildcraft.gui.fluid.empty") : fluid.getHoverName();
        List<Component> lines = List.of(Component.translatable(titleKey), name,
            Component.literal(tank.getAmountAsInt(0) + " / " + tank.getCapacity() + " mB"));
        graphics.setTooltipForNextFrame(font, lines, Optional.empty(), mouseX, mouseY);
    }

    private void drawIcon(GuiGraphicsExtractor graphics, int[] icon, int[] rect) {
        graphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, leftPos + rect[0], topPos + rect[1], icon[0], icon[1], rect[2],
            rect[3], 256, 256);
    }

    private void drawTank(GuiGraphicsExtractor graphics, Tank tank, int[] rect, int[] overlay) {
    ...
    static void drawFluid(GuiGraphicsExtractor graphics, Tank tank, int x, int y, int width, int height) {
        ...
        for (int ty = 0; ty < height; ty += 16) {
            for (int tx = 0; tx < width; tx += 16) {
                graphics.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, x + tx, y + ty, 16, 16, tint);
            }
        }
        graphics.disableScissor();
    }

    private static int fluidColour(FluidResource fluid) {
        if (fluid.isEmpty()) {
            return -1;
        }
        if (fluid.getFluidType() instanceof BCFluidType bc) {
        ...
        FluidModel model = Minecraft.getInstance().getModelManager().getFluidStateModelSet().get(fluidState);
        BlockTintSource tintSource = model.tintSource();
        return tintSource == null ? -1 : ARGB.opaque(tintSource.color(fluidState.createLegacyBlock()));
    }

    private double drawAnimation(GuiGraphicsExtractor graphics, int colourInput, int colourGas, int colourLiquid) {
        double distance = 0;
    ...
    private void drawAnimationLeftToRight(GuiGraphicsExtractor g, int colour, int[] icon, int[] rect, double startPoint) {
    ...
    private void drawAnimationBottomToTop(GuiGraphicsExtractor g, int colour, int[] icon, int[] rect, double startPoint) {
        double start = activeStart - startPoint;
        double end = activeEnd - startPoint;
        double size = rect[2] * rect[3];
        if (end < 0 || start >= size) {
    ...
    private void drawAnimationTopToBottom(GuiGraphicsExtractor g, int colour, int[] icon, int[] rect, double startPoint) {
        double start = activeStart - startPoint;
        double end = activeEnd - startPoint;
        double size = rect[2] * rect[3];
        if (end < 0 || start >= size) {
    ...
    private void drawAnimationPart(GuiGraphicsExtractor g, int colour, int[] icon, int[] rect, double u0, double v0,
        double u1, double v1) {
        ...
        g.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, leftPos + rect[0] + px0, topPos + rect[1] + py0, icon[0] + px0,
            icon[1] + py0, px1 - px0, py1 - py0, 256, 256, colour);
    }
}
