...
/**
 * Client-only menu screen registration for {@code buildcraft.factory}.
 *
 ...
 * is inherently client-only (a dedicated server never needs a {@code Screen}), so the correct, coarser fix is to
 * never let a dedicated server reach this class at all -- if it only ever ran through this guard, the bytecode
 * verifier never has a reason to resolve {@link GuiAutoCraftItems} (a client-only type, since {@code Screen} and
 * everything it touches doesn't exist on a dedicated server) in the first place.
 */
