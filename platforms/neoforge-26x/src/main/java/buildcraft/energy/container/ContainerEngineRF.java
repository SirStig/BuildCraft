...
/**
 * Slot layout, unchanged from 1.12.2's {@code ContainerEngineRF}: four upgrade slots at y=44. The RF-buffer level
 * indicator is a single container {@code DataSlot}, matching {@code ContainerEngineStone}'s own
 * {@code fuelPercent} precedent.
 */
public class ContainerEngineRF extends ContainerBCTile<TileEngineRF> {
    private final DataSlot rfPercent;

    public ContainerEngineRF(MenuType<?> type, int windowId, Inventory playerInv, TileEngineRF tile) {
    ...
    public ContainerEngineRF(int windowId, Inventory playerInv, RegistryFriendlyByteBuf extraData) {
        this(BCEnergyRegistries.ENGINE_RF_MENU.get(), windowId, playerInv, lookupTile(playerInv, extraData.readBlockPos()));
    }
