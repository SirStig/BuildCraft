 ...
 * <ul>
 * <li>density/viscosity/temperature/name go on the NeoForge {@link FluidType} ({@link BCFluidType});</li>
 * <li>flow distance and speed go on {@link BaseFlowingFluid.Properties}, which models them differently -- see
...
public final class BCEnergyFluids {
    ...
    private static final DeferredRegister<FluidType> FLUID_TYPES =
        DeferredRegister.create(NeoForgeRegistries.Keys.FLUID_TYPES, BuildCraft.MOD_ID);
    private static final DeferredRegister<Fluid> FLUIDS = DeferredRegister.create(Registries.FLUID, BuildCraft.MOD_ID);
    ...
    private static BCFluid defineFluid(BCRegistry registry, int[] data, int heat, String name) {
        ...
        Identifier still = Identifier.fromNamespaceAndPath(BuildCraft.MOD_ID, fluidTexture + "_still");
        Identifier flowing = Identifier.fromNamespaceAndPath(BuildCraft.MOD_ID, fluidTexture + "_flow");
        ...
        def.source = FLUIDS.register(fullName, () -> new BaseFlowingFluid.Source(def.fluidProperties(quanta,
            tempAdjustedViscosity)));
        def.flowing = FLUIDS.register("flowing_" + fullName, () -> new BaseFlowingFluid.Flowing(def.fluidProperties(
            quanta, tempAdjustedViscosity)));
        def.block = registry.addBlock(fullName, p -> new BCFluidBlock(def.source.get(), flammable, p), p -> {
            p.mapColor(mapColour)
                .replaceable()
                .noCollision()
                .strength(100.0F)
        ...
        def.bucket = registry.addItem(fullName + "_bucket",
            p -> new BCFluidBucketItem(def.source.get(), p.craftRemainder(Items.BUCKET).stacksTo(1)));
     ...
     * vanilla calibration is what this actually rests on.
     */
    ...
    public static final class BCFluid {
        ...
        private DeferredHolder<FluidType, BCFluidType> type;
        private DeferredHolder<Fluid, BaseFlowingFluid.Source> source;
        private DeferredHolder<Fluid, BaseFlowingFluid.Flowing> flowing;
        private DeferredBlock<BCFluidBlock> block;
        private DeferredItem<BCFluidBucketItem> bucket;
        ...
        private BaseFlowingFluid.Properties fluidProperties(int quanta, int viscosity) {
        ...
        public DeferredHolder<FluidType, BCFluidType> getType() {
            return type;
        }
        ...
        public DeferredBlock<BCFluidBlock> getBlock() {
            return block;
        }

        public DeferredItem<BCFluidBucketItem> getBucket() {
            return bucket;
        }
    }
}
