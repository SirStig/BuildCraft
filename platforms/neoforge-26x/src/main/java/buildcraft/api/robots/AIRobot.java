...
public class AIRobot {
    ...
    public final void loadFromNBT(CompoundTag nbt) {
        loadSelfFromNBT(nbt.getCompound("data").orElseGet(CompoundTag::new));

        if (nbt.contains("delegateAI")) {
            CompoundTag sub = nbt.getCompound("delegateAI").orElseGet(CompoundTag::new);

            try {
                Class<?> aiRobotClass;
                if (sub.contains("class")) {
                    // Migration support for 6.4.x
                    aiRobotClass = RobotManager.getAIRobotByLegacyClassName(sub.getStringOr("class", ""));
                } else {
                    aiRobotClass = RobotManager.getAIRobotByName(sub.getStringOr("aiName", ""));
                }
    ...
    public static AIRobot loadAI(CompoundTag nbt, EntityRobotBase robot) {
        ...
        try {
            ...
            if (nbt.contains("class")) {
                // Migration support for 6.4.x
                aiRobotClass = RobotManager.getAIRobotByLegacyClassName(nbt.getStringOr("class", ""));
            } else {
                aiRobotClass = RobotManager.getAIRobotByName(nbt.getStringOr("aiName", ""));
            }
            if (aiRobotClass != null) {
