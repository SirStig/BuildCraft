...
/** Ported from 1.12.2 with renames only -- {@code BlockPos.offset(EnumFacing)} became
 * {@link BlockPos#relative(Direction)}, and {@code EnumFacing.rotateAround(Axis)} became
 * {@link Direction#getClockWise(Axis)} (there is no longer a bare {@code rotateAround}; the modern API spells
 * the same 90-degree-clockwise-around-an-axis operation as a named direction instead). */
