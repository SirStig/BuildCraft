...
/** No fluid-stack deserializer on this target: {@code net.minecraftforge.fluids.FluidStack} is gone (see
 * PORTING.md's transfer-API divergence note), and 26.x's {@code FluidResource} has no equivalent
 * "resource + amount" carrier type that stands in for it in a standalone JSON deserializer, since amount only
 * makes sense in the context of a slot-indexed {@code ResourceHandler}. 1.20.1 keeps a
 * {@code FLUID_STACK_DESERIALIZER} -- see that copy of this file. */
