...
/** Walks every block position in a {@link Box} in a single, fixed axis order -- {@code buildcraft.builders.tile.
 * TileQuarry}'s "boustrophedon" dig sweep is just this iterator's natural traversal order, one full pass across two
 * axes before the third (outermost) axis advances.
 *
 * <p>Ported line-for-line from 1.12.2 (only {@code EnumFacing}/{@code EnumFacing.Axis} -> {@link Direction}/
 * {@link Direction.Axis}, {@code NBTTagCompound} -> {@link CompoundTag} and {@code BlockPos.ORIGIN} ->
 * {@link BlockPos#ZERO}): the arithmetic in {@link #advance()}/{@link #moveTo(BlockPos)} is unchanged. */
public class BoxIterator implements Iterator<BlockPos> {
    ...
    private boolean shouldReset(Direction facing) {
        int lastReturned = VecUtil.getValue(current, facing.getAxis());
        BlockPos goingTo = facing.getAxisDirection() == AxisDirection.POSITIVE ? max : min;
        int to = VecUtil.getValue(goingTo, facing.getAxis());
        if (facing.getAxisDirection() == AxisDirection.POSITIVE) return lastReturned > to;
        return lastReturned < to;
    }
    ...
    public boolean contains(BlockPos pos) {
        ...
        return pos.getZ() >= min.getZ() && pos.getZ() <= max.getZ();
    }
