...
public class NbtSquisher {
    // 1.12.2's Profiler was a concrete, instantiable class that did nothing unless explicitly enabled.
    // ProfilerFiller is an interface now; InactiveProfiler.INSTANCE is the equivalent standalone no-op
    // implementation, not tied to a running world or thread the way Minecraft.getInstance().getProfiler()
    // would be.
    ...
    public static Function<ByteBuf, RegistryFriendlyByteBuf> debugBuffer = null;
