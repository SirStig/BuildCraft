...
public class NbtSquishMap {
    ...
    // gnu.trove -> fastutil: the list-of-lists still relies on ByteArrayList/IntArrayList's content-based
    // equals()/hashCode() for the contains() dedup checks below, which fastutil provides just like Trove did.
    ...
    public int indexOfTag(Tag nbt) {
        ...
        if (nbt instanceof ByteTag) {
            return bytes.indexOf(((ByteTag) nbt).value());
        } else {
            offset += bytes.size();
        }
        if (nbt instanceof ShortTag) {
            return offset + shorts.indexOf(((ShortTag) nbt).value());
        } else {
            offset += shorts.size();
        }
        if (nbt instanceof IntTag) {
            return offset + ints.indexOf(((IntTag) nbt).value());
        } else {
            offset += ints.size();
        }
        if (nbt instanceof LongTag) {
            return offset + longs.indexOf(((LongTag) nbt).value());
        } else {
            offset += longs.size();
        }
        if (nbt instanceof FloatTag) {
            return offset + floats.indexOf(((FloatTag) nbt).value());
        } else {
            offset += floats.size();
        }
        if (nbt instanceof DoubleTag) {
            return offset + doubles.indexOf(((DoubleTag) nbt).value());
        } else {
            offset += doubles.size();
        }
        ...
        if (nbt instanceof StringTag) {
            return offset + strings.indexOf(((StringTag) nbt).value());
        } else {
            offset += strings.size();
        }
