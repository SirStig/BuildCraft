...
public class DeltaInt {
    ...
    void receiveData(EnumDeltaMessage type, PacketBufferBC buffer) {
    ...
    void writeState(PacketBufferBC buffer) {
    ...
    public void readFromNBT(CompoundTag nbt) {
        ...
        changingEntries.clear();
        nbt.getList("changing").ifPresent(list -> list.compoundStream().forEach(entryNbt -> {
            long start = entryNbt.getLongOr("start", 0);
            long end = entryNbt.getLongOr("end", 0);
