 ...
 *
 * <p>The interface itself is unaffected by the port -- it is pure data and geometry. What actually forces the
 * chunks stays load-bearing, {@code ChunkLoaderManager}, has not been ported: NeoForge's ticket API
 * ({@code ForgeChunkManager.requestTicket}/{@code Ticket}) was replaced by a controller-registered
 * {@code TicketController}/{@code TicketHelper} pair, and the same redesign had already landed in Forge before
 * 1.20.1 -- this needs rewriting on both targets, not porting on one and mechanically renaming on the other.
 * It follows once a machine (the quarry, the pump) actually needs it.
 */
