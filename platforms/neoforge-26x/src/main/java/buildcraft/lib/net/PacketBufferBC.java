 ...
 *
 * <p>Ported onto {@link FriendlyByteBuf} rather than {@link net.minecraft.network.RegistryFriendlyByteBuf}: the
 * bit-packing this class adds needs no registry access, and {@code RegistryFriendlyByteBuf} extends this class
 * on this target, so anything that does need registry-aware encoding wraps one of those around the same buffer
 * instead of this class trying to be both.
 *
 ...
 * ({@code IPayloadReceiver}, {@code MessageManager}) is not ported here. NeoForge payloads are registered
 * per-type up front through {@code StreamCodec}, a materially different shape from 1.12's single
 * dynamically-dispatched channel, and design for that follows once there is a concrete message to register
 * rather than being speculated on in the abstract.
 */
