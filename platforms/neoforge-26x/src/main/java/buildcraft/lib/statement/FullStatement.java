 ...
 *
 * <p>Every NBT/buffer method here gained a {@link HolderLookup.Provider}, following {@code StatementType}'s own
 * port -- see that class's javadoc. */
public class FullStatement<S extends IStatement> implements IReference<S> {
    ...
    public void readFromNbt(CompoundTag nbt, HolderLookup.Provider registries) {
        statement = type.readFromNbt(nbt.getCompoundOrEmpty("s"), registries);
        if (statement == null) {
            Arrays.fill(params, null);
        } else {
            for (int p = 0; p < params.length; p++) {
                CompoundTag pNbt = nbt.getCompoundOrEmpty(Integer.toString(p));
                params[p] = StatementTypeParam.INSTANCE.readFromNbt(pNbt, registries);
            }
        }
    }
    ...
    public CompoundTag writeToNbt(HolderLookup.Provider registries) {
        ...
        if (statement != null) {
            nbt.put("s", type.writeToNbt(statement, registries));
            for (int p = 0; p < params.length; p++) {
                IStatementParameter param = params[p];
                if (param != null) {
                    nbt.put(Integer.toString(p), StatementTypeParam.INSTANCE.writeToNbt(param, registries));
                }
            }
        }
        return nbt;
    ...
    public void readFromBuffer(PacketBufferBC buffer, HolderLookup.Provider registries) throws IOException {
        if (buffer.readBoolean()) {
            statement = type.readFromBuffer(buffer, registries);
            for (int p = 0; p < params.length; p++) {
                params[p] = StatementTypeParam.INSTANCE.readFromBuffer(buffer, registries);
            }
        } else {
    ...
    public void writeToBuffer(PacketBufferBC buffer, HolderLookup.Provider registries) {
        if (statement == null) {
            buffer.writeBoolean(false);
        } else {
            buffer.writeBoolean(true);
            type.writeToBuffer(buffer, statement, registries);
            for (int p = 0; p < params.length; p++) {
                IStatementParameter param = params[p];
                StatementTypeParam.INSTANCE.writeToBuffer(buffer, param, registries);
            }
        }
    }
