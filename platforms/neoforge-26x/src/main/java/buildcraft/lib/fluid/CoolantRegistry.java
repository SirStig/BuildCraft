 ...
 * enum singleton. As with {@link FuelRegistry}, a liquid coolant is keyed by a {@link FluidResource}, so 1.12.2's
 * "{@code fluid == null || fluid.amount == 0}" empty check becomes {@link FluidResource#isEmpty()}, and
 * {@code isFluidEqual} becomes {@link FluidResource#equals}. 1.12.2's {@code ItemStack#isItemEqual} (item and
 * metadata, count ignored) becomes {@link ItemStack#isSameItem} -- metadata no longer exists, and the item alone
 * is what the flattening folded it into.
 */
public enum CoolantRegistry implements ICoolantManager {
    ...
    @Override
    public ICoolant addCoolant(FluidResource fluid, float degreesCoolingPerMB) {
    ...
    @Override
    @Nullable
    public ICoolant getCoolant(FluidResource fluid) {
    ...
    @Override
    public float getDegreesPerMb(FluidResource fluid, float heat) {
        if (fluid == null || fluid.isEmpty()) {
            return 0;
        }
        for (ICoolant coolant : coolants) {
    ...
    public static class Coolant implements ICoolant {
        ...
        public Coolant(FluidResource fluid, float degreesCoolingPerMB) {
            this.fluid = fluid;
            this.degreesCoolingPerMB = degreesCoolingPerMB;
        }

        @Override
        public boolean matchesFluid(FluidResource stack) {
            return fluid.equals(stack);
        }

        @Override
        public float getDegreesCoolingPerMB(FluidResource stack, float heat) {
