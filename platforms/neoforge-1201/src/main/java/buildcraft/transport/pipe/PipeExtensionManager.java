 ...
 * {@code TickEvent.LevelTickEvent} {@code Phase.END} listener; this enum is registered onto
 * {@code MinecraftForge.EVENT_BUS} from {@code BCTransportRegistries} at exactly the spot 1.12.2's own
 * {@code BCTransportRegistries} registers it). Acting immediately instead would tear down the very
 ...
 * over RCON on the 26.x copy of this class: a fed void pipe item ends up placed one block ahead of the stripes
 * pipe with no relocation, leaving the stripes pipe with two connections (and consequently no open face at all)
 * rather than retracting. This is not new: every pipe item hit this same fallback before this pass, since the
 * previous stub declined unconditionally. Un-laying a run needs to walk backward along the pipe network
 * re-deriving each previous segment's own item and pipe definition, which is materially more bookkeeping than the
 * plain forward-extension case above; left as a clearly separate follow-up, at which point this fallback stops
 * applying to the void pipe too.
 */
public enum PipeExtensionManager implements IPipeExtensionManager {
    ...
    /** Runs every {@link #requestPipeExtension} queued for {@code event.level} once that level's own tile ticking
     * has already finished for this tick -- see this class's own javadoc for why this cannot run inline. */
    @SubscribeEvent
    public void onLevelTick(TickEvent.LevelTickEvent event) {
        if (event.phase != TickEvent.Phase.END || event.side != LogicalSide.SERVER) {
            return;
        }
        List<ExtensionRequest> pending = requests.remove(event.level);
        if (pending == null || pending.isEmpty() || !(event.level instanceof ServerLevel serverLevel)) {
            return;
        }
