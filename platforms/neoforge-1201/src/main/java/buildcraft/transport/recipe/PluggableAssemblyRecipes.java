 ...
 * duplicated here. See the 26.x copy of this class for the one real divergence: 1.20.1 has no
 * {@code ColorCollection}-style helper for the 16 stained glass blocks, so {@link #stainedGlass} is a plain
 * switch instead of {@code Blocks.STAINED_GLASS.pick(colour)}.
 *
...
public final class PluggableAssemblyRecipes {
    ...
    private static void registerLenses() {
        for (DyeColor colour : DyeColor.values()) {
            IngredientStack stainedGlass = new IngredientStack(Ingredient.of(stainedGlass(colour)));
    ...
    /** 1.20.1 has no {@code ColorCollection}-style lookup (that record is new to 26.x) -- the 16 stained glass
     * blocks are still 16 separate {@code Blocks} fields here, so this is a plain switch instead. */
    private static Block stainedGlass(DyeColor colour) {
