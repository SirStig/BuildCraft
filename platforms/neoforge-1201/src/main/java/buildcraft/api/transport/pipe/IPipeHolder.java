 ...
 *
 * <p>Two things changed beyond the type renames. {@code Capability<T>} is {@link Capability}, so
 * {@link #getCapabilityFromPipe} is keyed by a block capability and a side rather than by a capability alone.
 * And {@code PacketBuffer} is {@link FriendlyByteBuf} rather than plain {@code FriendlyByteBuf}: a
...
public interface IPipeHolder extends IRedstoneStatementContainer {
     ...
     */
    @Nullable
    <T> T getCapabilityFromPipe(Direction side, Capability<T> capability);
