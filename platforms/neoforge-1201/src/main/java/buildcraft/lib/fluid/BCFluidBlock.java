 ...
 * {@code IForgeBlock} overrides below (plus {@code ignitedByLava()} on the block properties, set by
 ...
 * {@code FluidType} now drives swimming/drag for every fluid generically); {@code renderLayer = SOLID} (a fluid with no
 * {@code ItemBlockRenderTypes} entry already renders in the solid layer on 1.20.1, and the recoloured sprites are
 * fully opaque); and the {@code sticky} web-like drag, which 1.12.2 only ever enabled behind the
...
public class BCFluidBlock extends LiquidBlock {
    ...
    /** Uses Forge's {@code Supplier} constructor (1.20.1 only -- 26.x's {@code LiquidBlock} takes the fluid itself),
     * the form Forge's own javadoc on it recommends for registry-object fluids. */
    public BCFluidBlock(Supplier<? extends FlowingFluid> fluid, boolean flammable, Properties properties) {
    ...
    public MutableComponent getName() {
        return getFluid().getFluidType().getDescription().copy();
    }
}
