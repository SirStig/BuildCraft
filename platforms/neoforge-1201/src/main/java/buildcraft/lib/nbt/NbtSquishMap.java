 ...
 *
 * <p>Reading a primitive tag's payload is {@code getAsByte()}/{@code getAsShort()}/etc on 1.20.1, where the
 * 26.x copy of this file uses {@code value()} instead -- Mojang renamed the accessor between the two
 * versions, so this is a genuine per-target divergence rather than a pure rename.
 */
