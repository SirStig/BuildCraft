 ...
 *
 * <p>1.12.2's {@code net.minecraft.util.text.translation.I18n} worked on both sides because it just read the
 * mod's own bundled {@code .lang} files. The client-only {@code I18n} in modern Minecraft (which additionally
 * reads the player's chosen resource packs) is not it -- {@link Language#getInstance()} is, and is common code
 * for the same reason the 1.12.2 class was: it falls back to whatever language file was loaded (the mod jar's
 * {@code en_us.json} on a dedicated server, the active resource pack on a client). */
public class LocaleUtil {
    ...
    public static String localizeMj(long mj) {
        if (BCLibConfig.powerMode == PowerMode.DISPLAY_RF) {
            return localizeRf((int) (mj / IMjToRfStatus.get().getConversion().mjPerRf));
        }
    ...
    public static String localizeMjFlow(long mj) {
        if (BCLibConfig.powerMode == PowerMode.DISPLAY_RF) {
            return localizeRfFlow((int) (mj / IMjToRfStatus.get().getConversion().mjPerRf));
        }
