...
/**
 * This is the 1.20.1 copy. It keeps the 1.12.2 read-and-mutate idiom for {@link #getItemData} unchanged,
 * because 1.20.1 still stores item NBT as a live {@link CompoundTag} on the stack ({@code getOrCreateTag()}).
 * The 26.x copy cannot do that -- item state is a data component there, copy-on-read -- so it splits into
 * {@code getItemData}/{@code setItemData}/{@code updateItemData}. See that copy's javadoc for the detail.
 */
public final class NBTUtilBC {
    ...
    public static Tag merge(@Nullable Tag destination, @Nullable Tag source) {
        ...
        if (destination.getId() == Tag.TAG_COMPOUND && source.getId() == Tag.TAG_COMPOUND) {
            ...
            Set<String> keys = Sets.union(destCompound.getAllKeys(), sourceCompound.getAllKeys());
            for (String key : keys) {
    ...
    public static <E extends Enum<E>> EnumSet<E> readEnumSet(@Nullable Tag tag, Class<E> clazz) {
        ...
        if (tag instanceof ByteTag byteTag) {
            bytes = new byte[] { byteTag.getAsByte() };
    ...
    public static Stream<CompoundTag> readCompoundList(@Nullable Tag list) {
        ...
        return IntStream.range(0, listTag.size()).mapToObj(listTag::getCompound);
    }
    ...
    public static Stream<String> readStringList(@Nullable Tag list) {
        ...
        return IntStream.range(0, listTag.size()).mapToObj(listTag::getString);
    }
}
