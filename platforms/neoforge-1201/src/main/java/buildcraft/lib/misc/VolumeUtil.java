 ...
 *
 * <p>{@code World.isAirBlock(pos)} has no direct successor; {@code level.getBlockState(pos).isAir()} is the
 * modern spelling. {@code BlockPos.offset(EnumFacing[, int])} is {@link BlockPos#relative}, and
 * {@code EnumFacing.getFacingFromAxis(AxisDirection, Axis)} swapped its argument order to become
 * {@link Direction#fromAxisAndDirection(Axis, AxisDirection)}. */
